...
import (
	"context"
	"github.com/capitalonline/cds-eci-profile/policy"
	"github.com/capitalonline/cds-eci-profile/resource"
	"github.com/capitalonline/cds-eci-profile/utils"
	"github.com/capitalonline/cds-eci-profile/webhook"
...
type Config struct {
	...
	ProfileClient interface{} // 检查现有ConfigMap
	CACertPath    string
	CAKeyPath     string
}
...
func NewManager(config *Config) (*Manager, error) {
	...
	resourceManager := resource.NewManager(config.K8sClient)
	manager := &Manager{
...
func (m *Manager) matchSelectorForPod(pod *v1.Pod) (*resource.Selector, error) {
	allSelectors, err := m.resourceManager.ListSelectors("kube-system", "eci-profile")
	if err != nil {
...
func (m *Manager) matchPod(selector *resource.Selector, pod *v1.Pod) (bool, error) {
	if selector.Spec.NamespaceLabels != nil {
		s, err := metav1.LabelSelectorAsSelector(selector.Spec.NamespaceLabels)
		if err != nil {
